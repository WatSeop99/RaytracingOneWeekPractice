...
bool IsScatter(in Material mat, in Ray ray, in Payload payload, in float3 normal, in float3 tangent, in uint hitKind, inout ScatterPayload scatterPayload)
{
	switch (mat.MatType)
	{
		case MATERIAL_LAMBERTIAN:
			{
				...
				float consineTheta = dot(normal, biTangent);
				scatterPayload.PDF = max(0.0f, consineTheta);
		...
		case MATERIAL_DIELECTRIC:
			{
				...
				float cosTheta = min(dot(-unitDir, normal), 1.0f);
