...
#define ALIGN(alignment, num) ((((num) + alignment - 1) / alignment) * alignment)
		...
		const UINT shaderRecordSizeInBytes = ALIGN(D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT, offsetToMaterialConstants + shaderRecordSize);

		m_shaderRecordSize = shaderRecordSizeInBytes;

		m_shaderRecords.reserve(numShaderRecords);
		UINT bufferSize = numShaderRecords * shaderRecordSizeInBytes;
