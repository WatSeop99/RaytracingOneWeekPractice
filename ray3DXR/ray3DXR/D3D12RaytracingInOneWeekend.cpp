...
void D3D12RaytracingInOneWeekend::SerializeAndCreateRaytracingRootSignature(D3D12_ROOT_SIGNATURE_DESC& desc, ID3D12RootSignature** ppRootSig)
{
	auto device = m_deviceResources->GetD3DDevice();
	...
	ThrowIfFailed(device->CreateRootSignature(1, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(ppRootSig)));
}
...
void D3D12RaytracingInOneWeekend::CreateRaytracingInterfaces()
{
	...
	ThrowIfFailed(device->QueryInterface(IID_PPV_ARGS(&m_pDXRDevice)), L"Couldn't get DirectX Raytracing interface for the device.\n");
	ThrowIfFailed(commandList->QueryInterface(IID_PPV_ARGS(&m_pDXRCommandList)), L"Couldn't get DirectX Raytracing interface for the command list.\n");
}
...
void D3D12RaytracingInOneWeekend::CreateLocalRootSignatureSubobjects(CD3DX12_STATE_OBJECT_DESC* raytracingPipeline)
{
	...
	localRootSignature->SetRootSignature(m_pRaytracingLocalRootSignature);
...
void D3D12RaytracingInOneWeekend::CreateRaytracingPipelineStateObject()
{
	...
	globalRootSignature->SetRootSignature(m_pRaytracingGlobalRootSignature);
...
void D3D12RaytracingInOneWeekend::CreateRaytracingOutputResource()
{
	...
	device->CreateUnorderedAccessView(m_pRaytracingOutput, nullptr, &UAVDesc, uavDescriptorHandle);
...
void D3D12RaytracingInOneWeekend::CreateDescriptorHeap()
{
	...
	device->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(&m_pDescriptorHeap));
...
void D3D12RaytracingInOneWeekend::BuildShaderTables()
{
	...
	UINT shaderIdentifierSize;
	{
		...
		ThrowIfFailed(m_pDXRStateObject->QueryInterface(IID_PPV_ARGS(&stateObjectProperties)));
...
void D3D12RaytracingInOneWeekend::DoRaytracing()
{
	...
	commandList->SetComputeRootSignature(m_pRaytracingGlobalRootSignature);

	// Copy the updated scene constant buffer to GPU.
	memcpy(&m_pMappedConstantData[frameIndex].constants, &m_frameCB[frameIndex], sizeof(m_frameCB[frameIndex]));
	auto cbGpuAddress = m_pPerFrameConstants->GetGPUVirtualAddress() + frameIndex * sizeof(m_pMappedConstantData[0]);
	commandList->SetComputeRootConstantBufferView(GlobalRootSignatureParams::SceneConstantSlot, cbGpuAddress);

	// Bind the heaps, acceleration structure and dispatch rays.
	commandList->SetDescriptorHeaps(1, &m_pDescriptorHeap);
	...
	DispatchRays(m_pDXRCommandList, m_pDXRStateObject, &dispatchDesc);
}
...
void D3D12RaytracingInOneWeekend::CopyRaytracingOutputToBackbuffer()
{
	...
	commandList->CopyResource(renderTarget, m_pRaytracingOutput);
...
{
	auto descriptorHeapCpuBase = m_pDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
...
UINT D3D12RaytracingInOneWeekend::CreateBufferSRV(D3DBuffer* buffer, UINT numElements, UINT elementSize)
{
	...
	buffer->gpuDescriptorHandle = CD3DX12_GPU_DESCRIPTOR_HANDLE(m_pDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), descriptorIndex, m_descriptorSize);
