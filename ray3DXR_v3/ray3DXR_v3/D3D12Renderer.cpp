...
BOOL CD3D12Renderer::Initialize(HWND hWnd, BOOL bEnableDebugLayer, BOOL bEnableGBV, BOOL bDebugShader, const WCHAR* wchShaderPath, DWORD dwMaxBlasCount)
{
	BOOL	bResult = FALSE;
	...
	m_fDPI = (float)GetDpiForWindow(hWnd);
...
lb_exit:
	...
	m_pShaderManager->Initialize(this, wchShaderPath, bDebugShader);

	CreateDepthStencil(m_dwWidth, m_dwHeight);

	// for raytracing
	m_pRayTracingManager = new CRayTracingManager;
	m_pRayTracingManager->Initialize(this, m_dwWidth, m_dwHeight, dwMaxBlasCount);
...
void CD3D12Renderer::UpdateCamera()
{
	...
	// view matrix
	m_matView = XMMatrixLookToLH(m_CamPos, m_CamDir, m_CamUp);
...
void CD3D12Renderer::SetCameraRot(float fYaw, float fPitch, float fRoll)
{
	m_fCamYaw += fYaw;
	m_fCamPitch += fPitch;
	m_fCamRoll += fRoll;
...
void CD3D12Renderer::EnableDXR(BOOL bSwitch)
{
	m_bDXREnabled = bSwitch;
}
BOOL CD3D12Renderer::IsEnabledDXR()
{
...
void CD3D12Renderer::CleanupDepthStencil()
{
	if (m_pDepthStencil)
	{
...
void CD3D12Renderer::FillRayTraceConstant(CONSTANT_BUFFER_RAY_TRACING* pOutBuffer)
{
	FillProjDecompConstant(&pOutBuffer->DecompProj);
	pOutBuffer->matViewInv = XMMatrixTranspose(m_matViewInv);

	XMMATRIX matViewProj = XMMatrixMultiply(m_matView, m_matProj);
	pOutBuffer->matViewProj = XMMatrixTranspose(matViewProj);

	//pOutBuffer->matViewProj = XMMatrixMultiplyTranspose(m_matView, m_matProj);
	
	pOutBuffer->CameraPosition = { m_CamPos.m128_f32[0], m_CamPos.m128_f32[1], m_CamPos.m128_f32[2], 1.0f };
	pOutBuffer->Near = NEAR_PLANE;
	pOutBuffer->Far = FAR_PLANE;
}

void CD3D12Renderer::FillProjDecompConstant(DECOMP_PROJ* pOutConstBuffer)
{
	
	//pOutConstBuffer->rcp_m11 = 1.0f / m_matProj._11;
	//pOutConstBuffer->rcp_m22 = 1.0f / m_matProj._22;
	//pOutConstBuffer->m21 = m_matProj._21;
	//pOutConstBuffer->m31 = m_matProj._31;
	//pOutConstBuffer->m32 = m_matProj._32;
	//pOutConstBuffer->m33 = m_matProj._33;
	//pOutConstBuffer->m43 = m_matProj._43;

	pOutConstBuffer->rcp_m11 = 1.0f / XMMatrixExtract(&m_matProj, 1, 1);
	pOutConstBuffer->rcp_m22 = 1.0f / XMMatrixExtract(&m_matProj, 2, 2);
	pOutConstBuffer->m21 = XMMatrixExtract(&m_matProj, 2, 1);
	pOutConstBuffer->m31 = XMMatrixExtract(&m_matProj, 3, 1);
	pOutConstBuffer->m32 = XMMatrixExtract(&m_matProj, 3, 2);
	pOutConstBuffer->m33 = XMMatrixExtract(&m_matProj, 3, 3);
	pOutConstBuffer->m43 = XMMatrixExtract(&m_matProj, 4, 3);
...
BOOL CD3D12Renderer::UpdateWindowSize(DWORD dwBackBufferWidth, DWORD dwBackBufferHeight)
{
	...
	m_pRayTracingManager->UpdateWindowSize(m_dwWidth, m_dwHeight);
...
void CD3D12Renderer::RenderMeshObject(void* pMeshObjHandle, const XMMATRIX* pMatWorld, BOOL bDeform, float fRadius)
{
	ID3D12GraphicsCommandList6* pCommandList = m_ppCommandList[m_dwCurContextIndex];
	CBasicMeshObject* pMeshObj = (CBasicMeshObject*)pMeshObjHandle;
	
	pMeshObj->Draw(pCommandList, pMatWorld, bDeform, fRadius);
}
...
void CD3D12Renderer::EndRender()
{
	...
	{	
		...
		// raytracing
		D3D12_CPU_DESCRIPTOR_HANDLE srvSky = {};
		if (m_pSkyTex)
		{
			srvSky = m_pSkyTex->srv;
		}
		
		m_pRayTracingManager->DoRaytracing(pCommandList, srvSky);


		// copy raytracing output to render target
		ID3D12Resource* pRayTracingOuputResource = m_pRayTracingManager->INL_GetOutputResource();
		...
		postCopyBarriers[0] = CD3DX12_RESOURCE_BARRIER::Transition(m_pRenderTargets[m_uiRenderTargetIndex], D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PRESENT);
...
void CD3D12Renderer::SetSkyCubeMap(void* pTexHandle)
{
	m_pSkyTex = (TEXTURE_HANDLE*)pTexHandle;
...
{
	// RayTracingManager에서 BLAS제거시 wait없이 처리하도록 구현했다 할지라도, BLAS에서 CBasicMeshObject에서 관리하고 있는 리소스를 참조하고 있으므로
	// 정확하게 no wait를 구현하려면(이하의 Wait를 제거하려면) CD3D12Renderer/CBasicMeshObject에서도 no wait를 처리해야한다.
...
void* CD3D12Renderer::CreateBLAS(void* pMeshObjHandle, BOOL bAllowUpdate)
{
	CBasicMeshObject* pMeshObj = (CBasicMeshObject*)pMeshObjHandle;
	void* pBlasHandle = pMeshObj->CreateBLAS(bAllowUpdate);
	return pBlasHandle;
}
void CD3D12Renderer::DeleteBLAS(void* pMeshObjHandle, void* pBlasHandle)
{
	CBasicMeshObject* pMeshObj = (CBasicMeshObject*)pMeshObjHandle;
	pMeshObj->DeleteBLAS(pBlasHandle);
}
void CD3D12Renderer::UpdateBLASTransform(void* pBlasHandle, const XMMATRIX* pMatWorld)
{
	//
	// BLAS의 강체변환을 위한 matrix를 변경.이에 따라 TLAS도 업데이트하도록 표시해둔다.
	// 이후에 CRayTracingManager::UpdateAccelerationStructure()에서 실제 TLAS업데이트(새로빌드)가 이루어진다.
	//
	BLAS_INSTANCE* pBlasInstance = (BLAS_INSTANCE*)pBlasHandle;
	m_pRayTracingManager->UpdateBLASTransform(pBlasInstance, pMatWorld);
}

void CD3D12Renderer::UpdateBLAS(void* pMeshObjHandle, void* pBlasHandle, const XMMATRIX* pMatWorld, float fRadius)
{
	//
	// VertextShader가 수행하는 vertex집합의 변형을 ComputeShader로 수행하고 그 결과를 BLAS가 참조하는 버퍼에 저장한다.
	//
	ID3D12GraphicsCommandList6* pCommandList = m_ppCommandList[m_dwCurContextIndex];
	...
	// ComputeShader를 이용한 vertex집합 가공은 즉시 이루어진다.
	pMeshObj->UpdateBLAS(pCommandList, pBlasInstance->uavVBResourceUpdated, pMatWorld, fRadius);
	
	// 해당 BLAS에 대해 업데이트가 필요함을 표시해둔다.
	// 이후에 CRayTracingManager::UpdateAccelerationStructure()에서 실제 BLAS업데이트가 이루어진다.
	m_pRayTracingManager->SetUpdateBLAS(pBlasInstance);
}
...
void* CD3D12Renderer::CreateImmutableTexture(UINT TexWidth, UINT TexHeight, DXGI_FORMAT format, const BYTE* pInitImage)
{
	TEXTURE_HANDLE* pTexHandle = m_pTextureManager->CreateImmutableTexture(TexWidth, TexHeight, format, pInitImage);
	return pTexHandle;
}
