...
HRESULT CD3D12ResourceManager::CreateIndexBuffer(DWORD dwIndexNum, D3D12_INDEX_BUFFER_VIEW* pOutIndexBufferView, ID3D12Resource **ppOutBuffer, void* pInitData, DWORD dwInitDataSize)
{
	HRESULT hr = S_OK;
	...
	ID3D12Resource*	pUploadBuffer = nullptr;
	UINT		IndexBufferSize = sizeof(DWORD) * dwIndexNum;
	...
	if (pInitData)
	{
		...
		memcpy(pIndexDataBegin, pInitData, dwInitDataSize);
		pUploadBuffer->Unmap(0, nullptr);
...
BOOL CD3D12ResourceManager::CreateTextureFromFile(ID3D12Resource** ppOutResource, D3D12_RESOURCE_DESC* pOutDesc, const WCHAR* wchFileName, BOOL* pbOutIsCubeMap)
{
	BOOL bResult = FALSE;
	...
	*pbOutIsCubeMap = FALSE;

	std::unique_ptr<uint8_t[]> ddsData;
	std::vector<D3D12_SUBRESOURCE_DATA> subresouceData;
	bool bIsCubeMap = false;
	if (FAILED(LoadDDSTextureFromFile(m_pD3DDevice, wchFileName, &pTexResource, ddsData, subresouceData, 0, nullptr, &bIsCubeMap)))
	{
		goto lb_return;
	}
