...
SHADER_HANDLE* CSpriteObject::m_pVS = nullptr;
SHADER_HANDLE* CSpriteObject::m_pPS = nullptr;
...
BOOL CSpriteObject::InitPipelineState()
{
	...
	m_pVS = pShaderManager->CreateShaderDXC(L"shSprite.hlsl", L"VSMain", L"vs_6_0", 0);
	if (!m_pVS)
		__debugbreak();
	m_pPS = pShaderManager->CreateShaderDXC(L"shSprite.hlsl", L"PSMain", L"ps_6_0", 0);
	if (!m_pPS)
		__debugbreak();
...
BOOL CSpriteObject::InitMesh()
{
	...
	if (FAILED(pResourceManager->CreateIndexBuffer((DWORD)_countof(Indices), &m_IndexBufferView, &m_pIndexBuffer, Indices, sizeof(Indices))))
	{
		__debugbreak();
		goto lb_return;
	}
