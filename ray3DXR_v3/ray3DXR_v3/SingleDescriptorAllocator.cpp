...
BOOL CSingleDescriptorAllocator::AllocDescriptorHandle(D3D12_GPU_DESCRIPTOR_HANDLE* pOutGPUHandle)
{
	BOOL	bResult = FALSE;
	...
	if (-1 != dwIndex)
	{
		CD3DX12_GPU_DESCRIPTOR_HANDLE DescriptorHandle(m_pHeap->GetGPUDescriptorHandleForHeapStart(), dwIndex, m_DescriptorSize);
		*pOutGPUHandle = DescriptorHandle;
		bResult = TRUE;
	}
	return bResult;
}
...
void CSingleDescriptorAllocator::FreeDescriptorHandle(D3D12_GPU_DESCRIPTOR_HANDLE DescriptorHandle)
{
	D3D12_GPU_DESCRIPTOR_HANDLE base = m_pHeap->GetGPUDescriptorHandleForHeapStart();
#ifdef _DEBUG
	if (base.ptr > DescriptorHandle.ptr)
		__debugbreak();
#endif
